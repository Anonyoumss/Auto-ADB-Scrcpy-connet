...
# Default paths for common installations on Windows
$defaultAdbPath = "C:\platform-tools" # Common location if extracted directly to C:
$defaultScrcpyPath = "C:\scrcpy\scrcpy-win64-v3.3.1" # Common location for scrcpy extraction
...
# Check if ADB executable is missing or Scrcpy executable is missing
if (-not (Test-Path "$adbPath\adb.exe") -or -not (Test-Path "$scrcpyPath\scrcpy.exe")) {
    Write-Host "Configuration paths not found or executables are missing." -ForegroundColor Yellow
    $useDefaults = Read-Host "Do you want to use default paths? (ADB: $defaultAdbPath, Scrcpy: $defaultScrcpyPath) [Y/N]"
    
    if ($useDefaults -eq 'y' -or $useDefaults -eq 'Y') {
        $adbPath = $defaultAdbPath
        $scrcpyPath = $defaultScrcpyPath
        Write-Host "Using default paths." -ForegroundColor Green
    } else {
